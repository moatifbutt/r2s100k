...
train_images = glob.glob(f"{config.ROOT_PATH}/2K-Merged/train/*")
train_images.sort()
train_segs = glob.glob(f"{config.ROOT_PATH}/2K-Merged/train_labels/*")
train_segs.sort()
valid_images = glob.glob(f"{config.ROOT_PATH}/RSD/val/*")
valid_images.sort()
valid_segs = glob.glob(f"{config.ROOT_PATH}/RSD/val_labels/*")
valid_images.sort()
...
train_image_transform = albumentations.Compose([
    albumentations.Resize(144, 256, always_apply=True),
